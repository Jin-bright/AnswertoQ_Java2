...
public class Test4 {
	...
	public void test() {
		...
		String search = sc.next();   // 검색할 문자 ( string 타입 ) 
		
		char searchCh = search.charAt(0);  // string->char
		int num = searchCh; // char  -> int 
		
		int count = 0; // 문자열 안에 있는 문자 수 세기 
		...
		for(int i=0; i<word.length(); i++) {
			if( word.charAt(i) == searchCh ) {
				count += 1;
			}
		}
		System.out.printf( "'%s'가 포함된 갯수 : %d개",search, count );
	}
